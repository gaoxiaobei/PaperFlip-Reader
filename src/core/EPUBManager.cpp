...
QString EPUBManager::readTextFromEntry(QuaZipFile& file)
{
    ...
    // Use QStringDecoder for Qt6 compatibility
    QStringDecoder decoder(QStringDecoder::Utf8);
    QString result = decoder.decode(data);
    
    // If decoding failed, try Latin-1 as fallback
    if (decoder.hasError()) {
        QStringDecoder latin1Decoder(QStringDecoder::Latin1);
        result = latin1Decoder.decode(data);
    }
